...
mod tests {
    ...
    use crate::generator::{encode_calldata_words, render_plank_program};
    use arbitrary::{Arbitrary, Unstructured};
    ...
    fn fixed_bytes_render_stably() {
        ...
        let first_source = render_plank_program(first_case.program());

        let mut second = Unstructured::new(&bytes);
        let second_case = GeneratedCase::arbitrary(&mut second).expect("case should decode");
        let second_source = render_plank_program(second_case.program());
